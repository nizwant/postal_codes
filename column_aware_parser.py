...
def extract_row_data(page, y_position: float, tolerance: float = 2.0) -> Optional[Dict[str, str]]:
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for y_pos in sorted_y:
            ...
            if row_data and row_data['left'] and row_data['right']:
                # Skip headers
                if any(keyword in row_data['left'] + row_data['right'] 
                ...
                if parsed:
                    ...
                    print(f"✓ {parsed['PNA']} | {parsed['Miejscowość']:20} | {parsed['Gmina']:15} | {parsed['Powiat']:12} | {parsed['Województwo']}")
                else:
                    print(f"✗ Could not parse: '{row_data['left']}' | '{row_data['right']}'")
        
        return results

def parse_full_pdf(pdf_path: str, output_csv: str = "postal_codes_column_aware.csv", 
    ...
    if all_results:
        ...
        for i, row in df.head(10).iterrows():
            print(f"  {row['PNA']} | {row['Miejscowość']:25} | {row['Gmina']:15} | {row['Powiat']:12} | {row['Województwo']}")
...
def main():
    pdf_file = "pages_3_to_22.pdf"  # Change this to your full PDF path when ready
